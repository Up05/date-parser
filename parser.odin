...
// combines date & time
// may swap date & time if components of both are zero
...
// formats a parsed date as: 'YYYY-MM-DD HH:MM:SS+HH:MM' (or 'Z' for UTC offset)
to_string :: proc(date: Date, time_sep := ' ') -> (out: string, err: DateError) { 
    date := date

    {
        using date
        if !between(year, 0, 9999) { return "", .YEAR_OUT_OF_BOUNDS }
        if !between(month, 0, 12) { return "", .MONTH_OUT_OF_BOUNDS }
        if !between(day, 0, days_in_month(year, month)) { return "", .DAY_OUT_OF_BOUNDS }
        if !between(hour, 0, 23) { return "", .HOUR_OUT_OF_BOUNDS }
        if !between(minute, 0, 59) { return "", .MINUTE_OUT_OF_BOUNDS }
        if !between(int(second), 0, 60) { return "", .SECOND_OUT_OF_BOUNDS }
        if !between(offset_hour, -23, 23) { return "", .OFFSET_HOUR_OUT_OF_BOUNDS }
        if !between(offset_minute, -59, 59) { return "", .OFFSET_MINUTE_OUT_OF_BOUNDS }
    }
...
// formats a parsed date as: 'YYYY-MM-DD HH:MM:SS+HH:MM' (skips time if is_date_only & date if is_time_only)
...
// quickly check if the input string looks like a date
...
// gets the index of the rune at which the parser failed (unoptimized, but not awfully slow)
...
// if timezone was local (date.is_date_local) just returns utc_offset as zero
